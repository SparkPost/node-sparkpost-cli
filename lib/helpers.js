...
function crudMap(keys, values, argv) {
  ...
  if (this.options && !_.isEmpty(this.options)) {
    ...
    const options = _.uniq(_.map(this.options, (rules, option) => option.split('.')[0]));
...
function crudSubcommands(crudOptions, module) {
  ...
  _.each(crudOptions.commands || [], (command) => {
    ...
    attachCrudCommandParameters(command, defaultCommand, crudOptions);
...
/**
 * Attaches the needs for id, options, and filters
 *
 */
/* eslint-disable complexity */
function attachCrudCommandParameters(command, defaultCommand, crudOptions) {
  ...
  if (_.includes(TAKES_OPTIONS, command)) {
    defaultCommand.options = crudOptions[`${command}_options`] || crudOptions.options || {};
  }

  if (_.includes(TAKES_FILTERS, command)) {
    defaultCommand.options = crudOptions[`${command}_filters`] || crudOptions.filters || {};
  }
}
