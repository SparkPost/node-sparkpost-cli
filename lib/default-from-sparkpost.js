...
module.exports = function(sparkpost, customCommands) {
  ...
  return _.map(customCommands, (module, endpoint) => {
    ...
    if (_.has(sparkpost, _.camelCase(endpoint)) && module.default) {
      const subcommands = generateSubcommands(sparkpost, module.command);
...
function generateSubcommands(sparkpost, endpoint) {
  const endpointCamel = _.camelCase(endpoint);

  const sparkpostCommands = _.keys(sparkpost[endpointCamel]);

  return _.keyBy(_.map(sparkpostCommands, (command) => {
    const commandCamel = command;

    command = _.kebabCase(command);
    const describe = generateSubcommandDescription(endpoint, command);
    const options = generateSubcommandOptions(sparkpost[endpointCamel][commandCamel]);
    const action = addAPIKeyCheck(sparkpost, endpointCamel, commandCamel);
    const usage = `Usage: $0 ${endpoint} ${command} [options]`;
...
function generateSubcommandDescription(endpoint, command) {
  ...
  if (_.includes(['list', 'search'], command)) {describe = `${_.lowerCase(endpoint)}`;}
...
function generateSubcommandOptions(func) {
  ...
  return _.keyBy(_.map(args, (option, index) => ({ name: option, index })), 'name');
}
